...
1. Establish what your research question
2. Gather data 
3. Methodology - map models 
4. Clean data/what variables - exploratory data analysis 
5. Choosing metrics 
6. Data split - training and testing
7. Build models 
    ...
    f. model selection 
8. Finalize the model (aka training it on whole dataset)
...
ggplot(data=t_df, aes(x=sex, y=survived)) + 
...
lasso_rec<-recipe(survived~.,data=surv_train_df) %>% 
  update_role(passenger_id, new_role = "ID") %>% #marker 
  step_rm(name,age) %>% #don't include name and age bc so many n/a(s)
...
log_wf <- workflow() %>%  #combine recipe and model 
  add_recipe(glm_rec) %>%
  add_model(log_md) 
...
```{r}
lasso_wf <- workflow() %>%  #combine recipe and model 
